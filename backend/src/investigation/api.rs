...
pub async fn delete_investigation_for_polling_station(
        ...
        delete_polling_station_investigation(conn, polling_station_id).await?
    {
        audit_service
            .log(
                conn,
                &AuditEvent::PollingStationInvestigationDeleted(serde_json::to_value(&investigation)?),
                None,
            )
            .await?;
...
async fn polling_station_investigation_conclude(
    ...
    audit_service
        .log(
            &mut tx,
            &AuditEvent::PollingStationInvestigationConcluded(serde_json::to_value(&investigation)?),
            None,
        )
        .await?;
