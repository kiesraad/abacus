...
export function ElectionStatusWithIcon(status: ElectionStatus, header: boolean, isAdministrator: boolean) {
  ...
  switch (status) {
    case "DataEntryInProgress":
      statusLabel = t(
        `election_status.first_session.${isAdministrator ? "coordinator" : "typist"}.data_entry_in_progress`,
      );
      break;
    case "FinishDataEntry":
      statusLabel = t("election_status.first_session.coordinator.finish_data_entry");
      break;
    case "DataEntryFinished":
      statusLabel = t(
        `election_status.first_session.${isAdministrator ? "coordinator" : "typist"}.data_entry_finished`,
      );
  }
  if (header) {
    return (
