...
const variants: TestVariant[] = [
  ...
  },
  {
    ...
    filename: "ab2023_rivierenpolder_gemeente_heemdamseburg",
  ...
  },
  {
    ...
    electionName: "Waterschap Rivier en Polder 2023",
    filename: "ab2023_rivierenpolder_gemeente_s-gravenveen",
  ...
  },
  {
    ...
    filename: "ps2023_oostholland_gemeente_juinen",
  ...
  },
  {
    ...
    filename: "ps2023_zuidbrabant_gemeente_middelgein",
...
for (const variant of variants) {
  test.describe(`full flow ${variant.name}`, () => {
    ...
    test("create GSB election and a new polling station", async ({ page }) => {
      ...
      if (variant.regionNumber) {
        const selectGSBPage = new SelectGSBPgObj(page);
        await expect(selectGSBPage.header).toBeVisible();
        await expect(selectGSBPage.regions).toHaveCount(4);
        await selectGSBPage.clickRegionFromList(variant.regionNumber);
      }
    ...
    test("download inlegvel", async ({ page }) => {
      await page.goto("/account/login");
      ...
      const expectedFilename =
        variant.countingMethod === "CSO" ? "Model_Na_31_2_Inlegvel.pdf" : "Model_Na_31_1_Inlegvel.pdf";
    ...
    for (const user of typistUsers) {
      test(`corrected data entry with ${user.fullname}`, async ({ page }) => {
        ...
        for (let i = 1; i < variant.dataEntry.political_group_votes.length; i++) {
    ...
    for (const typist of typistUsers) {
      test(`data entry for new polling station with ${typist.fullname}`, async ({ page }) => {
        ...
        await votersAndVotesPage.fillInPageAndClickNext(
          variant.dataEntry.voters_counts,
          variant.dataEntry.votes_counts,
        );

        const differencesPage = new DifferencesPage(page);
        await expect(differencesPage.fieldset).toBeVisible();
        await differencesPage.fillInPageAndClickNext(variant.dataEntry.differences_counts);

        await fillCandidatesListPages(page, variant.dataEntry);
    ...
    test("finish second session and download results", async ({ page }) => {
      ...
      const download = await downloadPromise;
      expect(download.suggestedFilename()).toMatch(
        new RegExp(`correctie_${variant.filename}-${dateTimePattern.source}.zip`),
      );
      expect((await stat(await download.path())).size).toBeGreaterThan(1024);
