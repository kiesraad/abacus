...
import { eml110a_PS2023_Limburg, eml110b_single, eml230b_PS2023_Limburg } from "e2e-tests/test-data/eml-files";
import { dataEntryPS2023_LimburgDSO } from "e2e-tests/test-data/specific-data-entries/PS2023_Limburg";
...
test.describe("full flow PS2 GSB DSO", () => {
  ...
  test("create GSB election and a new polling station", async ({ page }) => {
    ...
    await uploadElectionAndInputHash(page, eml110a_PS2023_Limburg);
    ...
    await uploadCandidatesAndInputHash(page, eml230b_PS2023_Limburg);
    ...
    await selectGSBPage.clickRegionFromList("0994");
    ...
    await expect(electionHomePage.header).toHaveText("Provinciale Staten Limburg 2023");
  ...
  test("start data entry", async ({ page }) => {
    ...
    await expect(electionHome.header).toHaveText("Provinciale Staten Limburg 2023");
    ...
    await expect(electionDetails.header).toHaveText("Gemeentelijk stembureau Valkenburg aan de Geul");
    await electionDetails.fillForm("Pannerdam", "18-03-2026", "21:34");

    await expect(electionHome.header).toContainText("Provinciale Staten Limburg 2023");
  ...
  for (const station of [
    ...
    test(`first data entry ${station.name}`, async ({ page }) => {
      ...
      await fillDataEntryPagesAndSave(page, dataEntryPS2023_LimburgDSO);
  ...
  test("finish first session and download results", async ({ page }) => {
    ...
    expect(download.suggestedFilename()).toMatch(
      /definitieve-documenten_ps2023_valkenburgaandegeul_gemeente_valkenburg-aan-de-geul-\d{8}-\d{6}.zip/,
    );
  ...
  for (const user of typistUsers) {
    test(`corrected data entry with ${user.fullname}`, async ({ page }) => {
      ...
      await firstCandidatesPage.fillCandidate(1, 63);
      await firstCandidatesPage.fillCandidate(2, 54);
      await firstCandidatesPage.fillCandidate(3, 7);
      await firstCandidatesPage.next.click();

      for (let i = 1; i < dataEntryPS2023_LimburgDSO.political_group_votes.length; i++) {
  ...
  for (const typist of typistUsers) {
    test(`data entry for new polling station with ${typist.fullname}`, async ({ page }) => {
      ...
      await votersAndVotesPage.fillInPageAndClickNext(
        dataEntryPS2023_LimburgDSO.voters_counts,
        dataEntryPS2023_LimburgDSO.votes_counts,
      );

      const differencesPage = new DifferencesPage(page);
      await expect(differencesPage.fieldset).toBeVisible();
      await differencesPage.fillInPageAndClickNext(dataEntryPS2023_LimburgDSO.differences_counts);

      await fillCandidatesListPages(page, dataEntryPS2023_LimburgDSO);
  ...
  test("finish second session and download results", async ({ page }) => {
    ...
    const download = await downloadPromise;
    expect(download.suggestedFilename()).toMatch(
      /correctie_ps2023_valkenburgaandegeul_gemeente_valkenburg-aan-de-geul-\d{8}-\d{6}.zip/,
    );
    expect((await stat(await download.path())).size).toBeGreaterThan(1024);
