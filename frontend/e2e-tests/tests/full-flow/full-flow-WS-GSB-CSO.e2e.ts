...
import { dataEntryAB2023_Limburg } from "e2e-tests/test-data/request-response-templates";
import type { TestUser } from "e2e-tests/test-data/users";
import { test } from "../../fixtures";
import { SelectGSBPgObj } from "../../page-objects/election/create/SelectGSBPgObj";
...
test.describe("full flow WS GSB CSO", () => {
  ...
  for (const station of [
    ...
    test(`first data entry ${station.name}`, async ({ page }) => {
      ...
      await fillDataEntryPagesAndSave(page, dataEntryAB2023_Limburg);
  ...
  for (const user of typistUsers) {
    test(`corrected data entry with ${user.fullname}`, async ({ page }) => {
      ...
      for (let i = 1; i < dataEntryAB2023_Limburg.political_group_votes.length; i++) {
  ...
  for (const typist of typistUsers) {
    test(`data entry for new polling station with ${typist.fullname}`, async ({ page }) => {
      ...
      await votersAndVotesPage.fillInPageAndClickNext(
        dataEntryAB2023_Limburg.voters_counts,
        dataEntryAB2023_Limburg.votes_counts,
      );

      const differencesPage = new DifferencesPage(page);
      await expect(differencesPage.fieldset).toBeVisible();
      await differencesPage.fillInPageAndClickNext(dataEntryAB2023_Limburg.differences_counts);

      await fillCandidatesListPages(page, dataEntryAB2023_Limburg);
