import { ApiClient, ApiError, ApiResponseStatus, FatalApiError, NetworkError, NotFoundError } from "@kiesraad/api";

import { LoginResponse, ValidationResultCode } from "@/types/generated/openapi";

export type ResultCode = ValidationResultCode | "REFORMAT_WARNING";

export type FieldValidationResult = {
  code: ResultCode;
  id: string;
  value?: string;
};

export type ErrorsAndWarnings = {
  errors: FieldValidationResult[];
  warnings: FieldValidationResult[];
};

export type AnyApiError = ApiError | FatalApiError | NetworkError | NotFoundError;

export type ApiResult<T, E = AnyApiError> = ApiResponse<T> | E;

export interface ServerError {
  error: string;
}

export type RequestMethod = "GET" | "POST" | "PUT" | "DELETE";

export interface ApiResponse<T> {
  status: ApiResponseStatus.Success;
  code: number;
  data: T;
}

export interface ApiState {
  client: ApiClient;
  user: LoginResponse | null;
  setUser: (user: LoginResponse | null) => void;
  logout: () => Promise<void>;
  login: (username: string, password: string) => Promise<ApiResult<LoginResponse>>;
}
