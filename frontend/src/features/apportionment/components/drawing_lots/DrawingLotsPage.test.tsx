...
describe("DrawingLotsPage", () => {
  ...
  test("Renders for drawing lots for list highest average variant and submits", async () => {
    ...
    expect(listitems[3]).toHaveTextContent(
      "Lijst 2 – Algemene Partij, Lijst 3 – KEUS, Lijst 4 – Algemene Lijst, Lijst 5 – Unie van kandidaten en Lijst 6 – Lijst van stemmers hebben ieder het hoogste gemiddeld aantal stemmen per zetel (46 2/3 stemmen)",
    );
  ...
  test("Renders for drawing lots for list largest remainder variant and submits", async () => {
    ...
    expect(listitems[3]).toHaveTextContent(
      "Lijst 2 – Politieke Groep der Kandidaten, Lijst 3 – Stemalliantie, Lijst 4 – Stem voor de Partij, Lijst 5 – Alliantie van Partijen en Lijst 6 – Unie voor Stemmen hebben ieder het grootste overschot (0 stemmen)",
    );
    expect(listitems[4]).toHaveTextContent("Daarom moet er geloot worden welke lijst de restzetel krijgt");
  ...
  test("Renders for drawing lots for candidate variant and submits", async () => {
    vi.spyOn(ReactRouter, "useNavigate").mockImplementation(() => navigate);
    const addCandidateDrawnRequestHandler = http.post("/api/elections/11/apportionment/add_candidate_drawn", () =>
      HttpResponse.json(
        {
          candidates_drawn: [],
          deceased_candidates: [],
    ...
    server.use(addCandidateDrawnRequestHandler);
    const addCandidateDrawn = spyOnHandler(addCandidateDrawnRequestHandler);
    overrideOnce(
      "get",
      "/api/elections/11",
      200,
    ...
    server.use(
      http.post("/api/elections/11/apportionment", () =>
        HttpResponse.json(
          {
    ...
    server.use(
      http.get("/api/elections/11/apportionment/state", () =>
        HttpResponse.json(lt19SeatsAndP15DrawingLots.state, { status: 200 }),
      ),
    );
    const user = userEvent.setup();

    renderDrawingLotsPage(11);

    expect(
      await screen.findByRole("heading", { level: 1, name: "Loting voor kandidaten met evenveel stemmen" }),
    ).toBeVisible();
    ...
    expect(navigate).toHaveBeenCalledWith("/elections/11/apportionment");
  });
