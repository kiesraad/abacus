...
describe("Fraction util", () => {
  test.each([
    [{ integer: 0, numerator: 3, denominator: 4 }, ""],
    [{ integer: 0, numerator: 0, denominator: 4 }, "0"],
    [{ integer: 1, numerator: 3, denominator: 4 }, "1"],
  ])("Fraction getFractionInteger fraction %s as %s", (input: DisplayFraction, expected) => {
    expect(getFractionInteger(input)).toBe(expected);
  });

  test.each([
    [{ integer: 0, numerator: 3, denominator: 4 }, "3/4"],
    [{ integer: 0, numerator: 0, denominator: 4 }, ""],
    [{ integer: 1, numerator: 3, denominator: 4 }, "3/4"],
