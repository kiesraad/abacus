...
4. Place the `abacus-windows-service.exe`, `install-service.exe`, and `uninstall-service.exe` from the same build into this folder (use `cargo build --workspace` to include these in the backend build).
5. Open `abacus.iss` with Inno Setup and select `Build -> Compile` in the menu or use the keyboard shortcut Ctrl+F9.

Running the installer as admin is required to set up the service.